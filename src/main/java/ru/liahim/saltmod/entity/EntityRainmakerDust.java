...
public class EntityRainmakerDust extends Entity {
  ...
  private int cloud = SaltConfig.cloudLevel.containsKey(Integer.valueOf(this.worldObj.provider.dimensionId)) ? ((Integer)SaltConfig.cloudLevel
    .get(Integer.valueOf(this.worldObj.provider.dimensionId))).intValue() : 128;
  ...
  public void onUpdate() {
    ...
    if (!this.worldObj.isRemote && this.lifeTime > 200 && this.posY >= this.cloud && !this.rain && !this.worldObj.getWorldInfo().isThundering()) {
      if (this.rand.nextInt(5) == 0 || this.worldObj.isRaining()) {
        MinecraftForge.EVENT_BUS.post((Event)new RainMakerEvent(this.worldObj, this.posX, this.posY, this.posZ, this.player, true));
      } else {
        MinecraftForge.EVENT_BUS.post((Event)new RainMakerEvent(this.worldObj, this.posX, this.posY, this.posZ, this.player, false));
      } 
      this.rain = true;
    } 
