...
public class SaltModEvent {
  ...
  public void onPlayerAttack(AttackEntityEvent e) {
    ...
    if (!world.isRemote && e.target != null && e.target instanceof EntityLivingBase) {
      ...
      Block block = null;
      if (is != null && EntityList.getEntityString((Entity)target) != null && ((
        EntityList.getEntityString((Entity)target).toLowerCase().contains("slime") && 
        !EntityList.getEntityString((Entity)target).toLowerCase().contains("lava")) || 
        EntityList.getEntityString((Entity)target).toLowerCase().contains("witch"))) {
        ...
        if (block != null && block == ModBlocks.saltCrystal) {
          ...
          if (!player.capabilities.isCreativeMode) {
            ...
            if (EntityList.getEntityString((Entity)target).toLowerCase().contains("witch"))
              player.addStat((StatBase)AchievSalt.saltWitch, 1); 
            if (target instanceof net.minecraft.entity.monster.EntitySlime) {
              ...
              EI.delayBeforeCanPickup = 10;
              world.spawnEntityInWorld((Entity)EIS);
              player.addStat((StatBase)AchievSalt.saltSlime, 1);
            } 
          } 
        } 
      } 
          ...
          if (player.getHealth() < player.getMaxHealth() && player.getFoodStats().getFoodLevel() > 0) {
            ...
            if (chek > 0) {
              ...
              if (chek == 6) {
                if (player.isBurning())
                  player.extinguish(); 
                event.player.addStat((StatBase)AchievSalt.fullMud, 1);
              } 
            } 
          } 
        } 
  ...
  public void addTempt(EntityJoinWorldEvent event) {
    if (event.entity instanceof EntityAnimal) {
      ...
      if (animal instanceof net.minecraft.entity.passive.EntityCow || animal instanceof net.minecraft.entity.passive.EntityHorse)
        animal.tasks.addTask(3, (EntityAIBase)new EntityAITempt((EntityCreature)animal, 1.25D, ModItems.salt, false)); 
    } 
  }
  ...
  public void addRain(RainMakerEvent event) {
    if (!event.world.isRemote) {
      ...
      if (event.player != null)
        event.player.addStat((StatBase)AchievSalt.rain, 1); 
    } 
  }
  ...
  public void itemPickup(EntityItemPickupEvent event) {
    ...
    if (!world.isRemote) {
      if (event.item.getEntityItem().getItem() == ModItems.salt)
        event.entityPlayer.addStat((StatBase)AchievSalt.salt, 1); 
      if (event.item.getEntityItem().getItem() == Item.getItemFromBlock(ModBlocks.saltCrystal))
        event.entityPlayer.addStat((StatBase)AchievSalt.saltCrystalGet, 1); 
      if (event.item.getEntityItem().getItem() == ModItems.mineralMud)
        event.entityPlayer.addStat((StatBase)AchievSalt.mineralMud, 1); 
      if (event.item.getEntityItem().getItem() == ModItems.saltWortSeed)
        event.entityPlayer.addStat((StatBase)AchievSalt.saltWort, 1); 
    } 
  }
  ...
  public void crafting(PlayerEvent.ItemCraftedEvent event) {
    if (event.crafting.getItem() == ModItems.mineralMud)
      event.player.addStat((StatBase)AchievSalt.mineralMud, 1); 
  }
