...
public class SlotExtractor extends Slot {
  ...
  public ItemStack decrStackSize(int par_1) {
    if (getHasStack())
      this.count += Math.min(par_1, (getStack()).stackSize); 
  ...
  protected void onCrafting(ItemStack stack) {
    ...
    if (!this.thePlayer.worldObj.isRemote) {
      ...
      while (i > 0) {
        ...
        i -= j;
        this.thePlayer.worldObj.spawnEntityInWorld((Entity)new EntityXPOrb(this.thePlayer.worldObj, this.thePlayer.posX, this.thePlayer.posY + 0.5D, this.thePlayer.posZ + 0.5D, j));
      } 
    } 
    this.count = 0;
    if (stack.getItem() == ModItems.saltPinch)
      this.thePlayer.addStat((StatBase)AchievSalt.moreBuckets, 1); 
  }
}