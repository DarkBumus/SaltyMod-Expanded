...
public class GuiExtractor extends GuiContainer {
  ...
  public GuiExtractor(InventoryPlayer player, TileEntityExtractor tileEntity) {
    super((Container)new ContainerExtractor(player, tileEntity));
    this.te = tileEntity;
  }
  ...
  protected void actionPerformed(GuiButton button) {
    CommonProxy.network.sendToServer((IMessage)new ExtractorButtonMessage(this.te.xCoord, this.te.yCoord, this.te.zCoord));
  }
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    if (mouseX >= w + 97 && mouseY >= h + 16 && mouseX < w + 97 + 3 && mouseY < h + 16 + 3) {
      ArrayList<String> toolTip = new ArrayList<String>();
      if (this.te.liquidLevel > 0)
        toolTip.add(I18n.format("container.discard", new Object[0])); 
      drawText(toolTip, mouseX, mouseY, this.fontRendererObj);
    } 
  }
