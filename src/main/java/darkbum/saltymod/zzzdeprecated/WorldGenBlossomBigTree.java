 ...
 * double d3 = Math.sqrt(Math.pow((double) Math.abs(this.basePos[0] - aint1[0]), 2.0) +
 * Math.pow(Math.abs(this.basePos[2] - aint1[2]), 2.0));
 ...
 * }
 * void func_150529_a(int p_150529_1_, int p_150529_2_, int p_150529_3_, float p_150529_4_, byte p_150529_5_, Block
 * p_150529_6_) {
 ...
 * if (!block1.isAir(this.worldObj, aint1[0], aint1[1], aint1[2]) && !block1.isLeaves(this.worldObj, aint1[0], aint1[1],
 * aint1[2])) {
 * ++j1;
 * } else {
 ...
 * return p_76495_1_ >= 0 && p_76495_1_ < this.leafDistanceLimit ? (p_76495_1_ != 0 && p_76495_1_ !=
 * this.leafDistanceLimit - 1 ? 3.0F : 2.0F) : -1.0F;
 * }
 ...
 * if(otherBlock != block && otherBlock != ModBlocks.blossom_burrow && otherBlock.isOpaqueCube() &&
 * !otherBlock.isLeaves(worldObj, finalPos[0], finalPos[1] - 1, finalPos[2])) {
 ...
 *//*
    ...
    * boolean isSoil = block.canSustainPlant(this.worldObj, this.basePos[0], this.basePos[1] - 1, this.basePos[2],
    * ForgeDirection.UP, (BlockSapling)ModBlocks.blossom_sapling);
    * if (!isSoil) {
    * return false;
    * } else {
