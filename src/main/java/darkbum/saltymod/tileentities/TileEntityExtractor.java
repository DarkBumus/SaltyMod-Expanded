...
import darkbum.saltymod.blocks.BlockExtractor;
...
public class TileEntityExtractor extends TileEntity implements ISidedInventory, IFluidHandler {
    ...
    public void updateEntity() {
        if (!this.worldObj.isRemote) {
            ...
            boolean liquid = (FluidRegistry.lookupFluidForBlock(blockUp) != null || blockUp instanceof net.minecraft.block.BlockDynamicLiquid);
            if (liquid) {
                ...
                if (blockFluid != null && !blockFluid.isGaseous() && this.worldObj
                    .getBlockMetadata(this.xCoord, this.yCoord + 1, this.zCoord) == 0) {
                    int den = blockFluid.getViscosity() / 200;
                    if (this.liquidLevel == 0 || (this.maxCap - this.liquidLevel > den && blockFluid ==
                        FluidRegistry.getFluid(this.liquidID))) {
                    ...
                    if (this.burningTime > 0) {
                        ...
                        if (this.invSlots[1] != null) {
                            ...
                            if ((this.invSlots[1]).stackSize == 0)
                                this.invSlots[1] = this.invSlots[1].getItem().getContainerItem(this.invSlots[1]);
                        }
                    }
                }
    ...
    public boolean canExtract() {
        ...
        if (this.invSlots[0] == null)
            return true;
        if (!this.invSlots[0].isItemEqual(itemstack))
            return false;
